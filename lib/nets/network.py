...
class Network(object):
    ...
    def _reshape_layer(self, bottom, num_dim, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            # bottom shape (1, H/16, W/16, 9 * 2)
            to_caffe = tf.transpose(bottom, [0, 3, 1, 2])
            # shape (1, 9 * 2, H/16, W/16)

            # then force it to have channel 2
            reshaped = tf.reshape(to_caffe,
                                  tf.concat(axis=0, values=[[1, num_dim, -1], [input_shape[2]]]))
            # array([ 1, 2, 9*14, 14])  怎么个意思
    ...
    def _softmax_layer(self, bottom, name):
        if name.startswith('rpn_cls_prob_reshape'):
            input_shape = tf.shape(bottom)  # (1, 9*14, 14, 2)
            bottom_reshaped = tf.reshape(bottom, [-1, input_shape[-1]])  # (9*14*14, 2)
    ...
    def _proposal_layer(self, rpn_cls_prob, rpn_bbox_pred, name):
        ...
        :param rpn_cls_prob: (1, 14, 14, 9*2)
        :param rpn_bbox_pred: (1, 14, 14, 9*4)
        :param name:
        :return:
        """
        with tf.variable_scope(name) as scope:
            if cfg.USE_E2E_TF:
                rois, rpn_scores = proposal_layer_tf(
                    rpn_cls_prob,
                    rpn_bbox_pred,
                    self._im_info,  # image's shape
                    self._mode,
                    self._feat_stride,   # [16, ...]
                    self._anchors,       # ndarray
                    self._num_anchors    # 9
                )
            else:
    ...
    def _anchor_component(self):
        with tf.variable_scope('ANCHOR_' + self._tag) as scope:
            ...
            if cfg.USE_E2E_TF:
                ...
                # anchors shape shape (1764, 4), anchor_length 1764
            else:
    ...
    def _region_proposal(self, net_conv, is_training, initializer):
        ...
        rpn = slim.conv2d(net_conv, cfg.RPN_CHANNELS, [3, 3], trainable=is_training, weights_initializer=initializer,
                          scope="rpn_conv/3x3")
        # rpn shape (1, H/16, W/16, cfg.RPN_CHANNELS)

        self._act_summaries.append(rpn)
        # 每个图像上的anchor得到两个分值(foreground, background)

        # shape (1, H/16, W/16, 9 * 2) num_anchors = 9
        ...
        # rpn_cls_score_reshape shape (1, 9*14, 14, 2)
        ...
        self._predictions["rpn_cls_score"] = rpn_cls_score  # (1, H/16, W/16, 9*2) 代表
        self._predictions["rpn_cls_score_reshape"] = rpn_cls_score_reshape  # (1, 9*14, 14, 2)
        self._predictions["rpn_cls_prob"] = rpn_cls_prob  # # shape (1, 14, 14, 9*2)
        self._predictions["rpn_cls_pred"] = rpn_cls_pred  # shape (9*14*14,)
